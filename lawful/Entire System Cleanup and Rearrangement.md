# Clean-up & Flounder Wallet Conceo 
...
Let's search the web for the latest updates in MetaMask, BASE and Multisig wallet connection for development purpose.
...
### Rebranding and Performance Improvements
...
**Cost Efficiency**: TotalSig's multisignature transactions incur up to 20x less in network fees compared to any other wallet, making MPC technology previously available only to large institutional players now accessible to everyone [Totalsig](https://www.totalsig.com/wallets/ethereum-multisig-wallet)  [Totalsig](https://www.totalsig.com/) .
...
**Ethereum Network**: On Ethereum, MPC multisigs require only one signature per transaction, bypassing expensive smart contract deployment and supporting ERC20 and ERC721 standards.
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center space-x-2">
              ...
              <span className="text-gray-300">Auto-scaling capabilities</span>
            </div>
          </div>
        </div>
      </div>
...
How do native Blockchain coins get timestamped on Coinbase.

E.g. custom Blockchain like CosmoSDK

When native blockchain coins from custom blockchains (like those built with Cosmos SDK) are listed on Coinbase, the timestamping process involves several key components:

## Blockchain-Level Timestamping

**Block Headers**: Each block on the native blockchain contains a timestamp field that records when the block was proposed/created. For Cosmos SDK chains, this follows the Tendermint consensus protocol, where validators include timestamps in block proposals.

**Transaction Timestamps**: Individual transactions inherit the timestamp of the block they're included in, providing an immutable record of when each transaction occurred on the native chain.

## Coinbase Integration Process

**Node Infrastructure**: Coinbase runs full nodes for each supported blockchain to directly read block data and transaction history. For Cosmos SDK chains, they'd run the specific chain's daemon (like `gaiad` for Cosmos Hub).

**Indexing Service**: Coinbase's backend systems continuously monitor new blocks, extracting transaction data including timestamps, amounts, addresses, and other metadata.

**Database Storage**: This blockchain data gets normalized and stored in Coinbase's internal databases, where the original blockchain timestamps are preserved alongside Coinbase's own processing timestamps.

## API and Display

**Timestamp Preservation**: When you view transaction history on Coinbase, the timestamps shown typically reflect the original blockchain timestamp, not when Coinbase processed or detected the transaction.

**Multiple Time References**: Coinbase may track several timestamps:
- Original blockchain timestamp
- When their systems first detected the transaction  
- When the transaction reached sufficient confirmations
- When it was credited to user accounts

The key point is that the authoritative timestamp comes from the native blockchain itself through the consensus mechanism (Tendermint in Cosmos SDK's case), and Coinbase preserves this original timing data in their systems.
